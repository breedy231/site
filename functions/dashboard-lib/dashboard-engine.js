...
class GridSystem {
  constructor(width, height, options = {}) {
    ...
    this.cellWidth =
      (this.width - 2 * this.margin - (this.cols - 1) * this.gap) / this.cols
    this.cellHeight =
      (this.height - 2 * this.margin - (this.rows - 1) * this.gap) / this.rows
  }
...
class ClockComponent extends ComponentBase {
  ...
  render(ctx, bounds) {
    ...
    const secondsHeight = this.config.showSeconds
      ? Math.round(timeHeight * this.config.secondsSize)
      : 0
    const totalHeight =
      timeHeight + (this.config.showSeconds ? secondsHeight + 10 : 0)
    ...
    if (this.config.showSeconds) {
      ...
      ctx.font = `${this.config.fontWeight} ${Math.round(
        originalSize * this.config.secondsSize
...
class StatsComponent extends ComponentBase {
  ...
  render(ctx, bounds) {
    ...
    // Title
    const originalSize = this.config.fontSize
    ctx.font = `bold ${Math.round(originalSize * this.config.titleSize)}px ${
...
class DeviceStatsComponent extends ComponentBase {
  ...
  render(ctx, bounds) {
    ...
    if (this.config.deviceStats) {
      ...
      // Temperature
      if (
        this.config.showTemperature &&
        deviceStats.temperature &&
        deviceStats.temperature !== "unknown"
      ) {
      ...
      if (deviceStats.system) {
        if (
          this.config.showUptime &&
          deviceStats.system.uptime_hours !== "unknown"
        ) {
          stats.push(`Uptime: ${deviceStats.system.uptime_hours}h`)
        }
        if (
          this.config.showMemory &&
          deviceStats.system.memory_usage_percent !== "unknown"
        ) {
      ...
      if (this.config.showWifi && deviceStats.wifi) {
        if (deviceStats.wifi.status !== "unknown") {
          ...
          if (
            deviceStats.wifi.network &&
            deviceStats.wifi.network !== "unknown" &&
            deviceStats.wifi.network !== "none"
          ) {
      ...
      // Last update
      if (
        this.config.showLastUpdate &&
        deviceStats.dashboard &&
        deviceStats.dashboard.last_update &&
        deviceStats.dashboard.last_update !== "unknown"
      ) {
...
class WeatherComponent extends ComponentBase {
  ...
  render(ctx, bounds) {
    ...
    if (this.config.showCurrent && weather.current) {
      ...
      ctx.font = `bold ${Math.round(originalSize * 1.2)}px ${
        this.config.fontFamily
      }`
      ...
      ctx.fillText(
        `${weatherSymbol} ${current.temperature}`,
        contentBounds.x,
        currentY
      )

      // Condition on same line
      const tempWidth = ctx.measureText(
        `${weatherSymbol} ${current.temperature}`
      ).width
      ctx.font = `${this.config.fontWeight} ${originalSize}px ${this.config.fontFamily}`
      ctx.fillText(
        ` ${current.condition}`,
        contentBounds.x + tempWidth + 5,
        currentY
      )
    ...
    if (
      ...
      ctx.font = `bold ${Math.round(originalSize * 1.1)}px ${
        this.config.fontFamily
      }`
...
class TitleComponent extends ComponentBase {
  ...
  render(ctx, bounds) {
    ...
    const textY =
      contentBounds.y + (contentBounds.height - this.config.fontSize) / 2
    ctx.fillText(
      this.config.text,
      contentBounds.x + contentBounds.width / 2,
      textY
